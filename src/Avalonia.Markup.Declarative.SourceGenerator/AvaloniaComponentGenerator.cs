...
public class AvaloniaComponentGenerator : IIncrementalGenerator
{
        ...
        while (currentType != null && currentType.SpecialType != SpecialType.System_Object)
        {
            ...
            foreach (var member in members)
            {
                ...
                if (hasInject)
                {
                    ...
                    {
                        injectables.Add((prop.Name, prop.Type.ToDisplayString(MarkupTypeHelpers.FullSymbols), true, !prop.IsReadOnly));
                    }
                    else if (member is IFieldSymbol field)
                    {
                        injectables.Add((field.Name, field.Type.ToDisplayString(MarkupTypeHelpers.FullSymbols), false, !field.IsReadOnly));
                    }
                }
                ...
                if (hasObserve)
                {
                    if (member is IPropertySymbol prop)
                    {
                        observables.Add((prop.Name, prop.Type.ToDisplayString(MarkupTypeHelpers.FullSymbols), true));
                    }
                    else if (member is IFieldSymbol field)
                    {
                        observables.Add((field.Name, field.Type.ToDisplayString(MarkupTypeHelpers.FullSymbols), false));
                    }
                }
                ...
                if (hasDeepObserve)
                {
                    if (member is IFieldSymbol field)
                    {
                        ...
                        deepObservables.Add((field.Name, field.Type.ToDisplayString(MarkupTypeHelpers.FullSymbols), isNotifier, field.Type.IsValueType));
    ...
    private static void GenerateSource(SourceProductionContext context, (ClassDeclarationSyntax Syntax, GeneratorData Data)? data)
    {
        ...
        sb.AppendLine("#pragma warning disable IL2026 // RequiresUnreferencedCode");
        sb.AppendLine("#pragma warning disable CS8620 // Nullability/Array covariance");
        sb.AppendLine("#pragma warning disable CS0105 // Duplicate using directive");
        sb.AppendLine();
        ...
        if (deepObservables.Count > 0)
        {
            ...
            sb.AppendLine("    // Generated Observable Binders");
            sb.AppendLine();
            sb.AppendLine("    extension<TComponent>(TComponent component)");
            sb.AppendLine($"        where TComponent : {typeName}");
            sb.AppendLine("    {");
            ...
            context.AddSource($"{uniqueName}.Component.Extensions.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
        }
    }
